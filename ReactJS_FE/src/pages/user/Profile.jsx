...
const Profile = () => {
    ...
    return (
        <div className="min-h-screen bg-[var(--theme-bg)] flex flex-col font-sans text-[var(--theme-text)]">
            <Header />
            ...
            <main className="flex-1 py-10">
                <div className="max-w-5xl w-full mx-auto px-4">
                    ...
                    {/* Page Header */}
                    <div className="mb-8">
                        <h1 className="text-3xl font-black uppercase tracking-[0.18em] mb-2" style={{ fontFamily: 'Anton, sans-serif' }}>Account Settings</h1>
                        <p className="text-white/55">Manage your personal information, security, and addresses.</p>
                    </div>

                    {msg && (
                        <div className={`mb-6 p-4 rounded-lg font-medium text-sm flex items-center justify-between ${msgType === 'success' ? 'bg-emerald-500/10 text-emerald-200 border border-emerald-500/20' : 'bg-red-500/10 text-red-200 border border-red-500/20'}`}>
                            {msg}
                    ...
                    {/* Avatar Card */}
                    <div className="bg-[rgba(255,255,255,0.04)] rounded-[28px] p-6 shadow-[0_20px_60px_rgba(0,0,0,0.28)] border border-white/10 mb-6 flex flex-col sm:flex-row items-center justify-between gap-6 backdrop-blur-sm">
                        <div className="flex items-center gap-5">
                            <div className="relative">
                                <div
                                    className="w-20 h-20 rounded-full overflow-hidden border border-white/10 bg-black/30 flex items-center justify-center"
                                    style={{ background: avatarPreview ? "transparent" : "#e5e7eb" }}
                                >
                                    {avatarPreview ? (
                                        <img src={avatarPreview} alt="avatar" className="w-full h-full object-cover" />
                                    ) : (
                                        <svg className="w-10 h-10 text-white/35" fill="currentColor" viewBox="0 0 24 24"><path d="M24 20.993V24H0v-2.996A14.977 14.977 0 0112.004 15c4.904 0 9.26 2.354 11.996 5.993zM16.002 8.999a4 4 0 11-8 0 4 4 0 018 0z" /></svg>
                                    )}
                                </div>
                        ...
                        <div className="flex gap-3">
                            <button onClick={() => fileInputRef.current?.click()} className="px-4 py-2 bg-[var(--theme-accent)] text-black text-sm font-black rounded-lg hover:brightness-110 transition-colors uppercase tracking-[0.18em]">
                                Upload New Photo
                            </button>
                            <button onClick={handleRemoveAvatar} className="px-4 py-2 bg-white/5 border border-white/10 text-white text-sm font-semibold rounded-lg hover:bg-white/10 transition-colors">
                                Remove
                            </button>
                        </div>
                    </div>
