...
const STATUS_STEPS = [
    {
        ...
        label: "Đơn hàng mới",
        icon: "📋",
        desc: "Đơn hàng của bạn đã được tiếp nhận",
    },
    {
        key: "confirmed",
        label: "Đã xác nhận",
    ...
    },
    {
        ...
        label: "Đang chuẩn bị",
        icon: "📦",
        desc: "Shop đang đóng gói sản phẩm",
    },
    {
        key: "shipping",
...
const STATUS_CONFIG = {
    pending: {
        label: "Đơn hàng mới",
        color: "bg-[#ffd500] text-black border-2 border-black shadow-[2px_2px_0_rgba(0,0,0,1)]",
        dot: "hidden",
    },
    confirmed: {
        label: "Đã xác nhận",
        color: "bg-[#00e5ff] text-black border-2 border-black shadow-[2px_2px_0_rgba(0,0,0,1)]",
        dot: "hidden",
    },
    preparing: {
        label: "Đang chuẩn bị",
        color: "bg-[#ff8c00] text-black border-2 border-black shadow-[2px_2px_0_rgba(0,0,0,1)]",
        dot: "hidden",
    },
    shipping: {
        label: "Đang giao hàng",
        color: "bg-[#b026ff] text-white border-2 border-black shadow-[2px_2px_0_rgba(0,0,0,1)]",
        dot: "hidden",
    },
    delivered: {
        label: "Đã giao thành công",
        color: "bg-[#52c41a] text-white border-2 border-black shadow-[2px_2px_0_rgba(0,0,0,1)]",
        dot: "hidden",
    },
    cancelled: {
        label: "Đã hủy",
        color: "bg-[#ff4d4f] text-white border-2 border-black shadow-[2px_2px_0_rgba(0,0,0,1)]",
        dot: "hidden",
    },
    cancel_requested: {
        label: "Yêu cầu hủy đang chờ",
        color: "bg-[#faad14] text-black border-2 border-black shadow-[2px_2px_0_rgba(0,0,0,1)]",
        dot: "hidden",
    },
    refunded: {
        label: "Đã hoàn tiền",
        color: "bg-[#d9d9d9] text-black border-2 border-black shadow-[2px_2px_0_rgba(0,0,0,1)]",
        dot: "hidden",
    },
};
    ...
    const handleCancel = async () => {
        ...
        try {
            const res = await fetch(`${API_URL}/api/orders/${id}/cancel`, {
                ...
                body: JSON.stringify({
                    reason: cancelReason || "Người dùng hủy đơn",
                }),
            });
            const data = await res.json();
    ...
    return (
        <div className="min-h-screen bg-[#e5e5e5] font-oswald flex flex-col relative">
            ...
            <main className="flex-1 max-w-[1400px] mx-auto px-6 pt-[90px] md:pt-[100px] pb-12 w-full z-10">
                ...
                <button
                    onClick={() => navigate("/orders")}
                    className="flex items-center gap-2 text-xs font-bold uppercase tracking-widest text-black/60 hover:text-black mb-8 transition-colors"
                >
                    <svg
                ...
                <div className="flex flex-col md:flex-row md:items-start justify-between mb-8 gap-4">
                    <div>
                        <h1 className="text-4xl lg:text-5xl font-anton uppercase tracking-widest text-black">
                            Đơn hàng #{order.id}
                        </h1>
                    ...
                    <span
                        className={`px-4 py-2 text-xs font-bold uppercase tracking-widest flex items-center gap-2 ${sc.color}`}
                    >
                        {sc.label}
                    </span>
                </div>
                ...
                {cancelMsg && (
                    <div
                        className={`mb-8 p-4 border-2 border-black shadow-[4px_4px_0_rgba(0,0,0,1)] flex items-start gap-3 text-sm font-bold uppercase tracking-widest text-white ${cancelMsg.includes("thành công") || cancelMsg.includes("gửi") ? "bg-[#52c41a]" : "bg-[#ff4d4f]"}`}
                    >
                        <span>
                            {cancelMsg.includes("thành công") ||
                            cancelMsg.includes("gửi")
                                ? "✅"
                                : "⚠️"}
                        </span>
                        <span>{cancelMsg}</span>
                ...
                <div className="flex gap-6">
                        ...
                        order.status !== "cancel_requested" ? (
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                {idx <
                                                    ...
                                                    <div
                                                        className={`absolute left-4 top-10 w-0.5 h-full ${isCompleted ? "bg-[var(--theme-accent)]" : "bg-black/10"}`}
                                                    />
                                                )}
                                                {/* Icon Circle */}
                                                <div
                                                    className={`w-9 h-9 border-2 flex items-center justify-center flex-shrink-0 text-sm z-10 transition-all font-bold ${
                                                        isActive
                                                ...
                                                <div className="flex-1 pt-1.5">
                                                    <p
                                                        className={`text-sm font-bold uppercase tracking-widest ${isActive ? "text-[var(--theme-accent)]" : isCompleted ? "text-black" : "text-black/40"}`}
                                                    >
                                                        {step.label}
                                                    </p>
                            ...
                            <div className="bg-white border-2 border-black shadow-[4px_4px_0_rgba(0,0,0,1)] p-8">
                                <div className="flex items-center gap-4">
                                    <div
                                        className={`w-14 h-14 border-2 border-black shadow-[2px_2px_0_rgba(0,0,0,1)] flex items-center justify-center text-2xl flex-shrink-0 ${order.status === "cancelled" ? "bg-[#ff4d4f]" : "bg-[#faad14]"}`}
                                    >
                                        {order.status === "cancelled"
                                            ? "❌"
                                            : "⏳"}
                                    ...
                                    <div>
                                        <p
                                            className={`font-bold text-lg uppercase tracking-widest ${order.status === "cancelled" ? "text-[#ff4d4f]" : "text-[#faad14]"}`}
                                        >
                                            {order.status === "cancelled"
                                                ? "Đơn hàng đã bị hủy"
                                                : "Yêu cầu hủy đang chờ xử lý"}
                                        </p>
                                        {order.cancelReason && (
                            ...
                            <div className="space-y-4">
                                {(order.items || []).map((item) => {
                                    ...
                                    return (
                                        <div
                                            key={item.id}
                                            className="p-4 border-2 border-black hover:bg-black/5 transition-colors space-y-4 bg-white"
                                        >
                                            <div className="flex items-center gap-4">
                                                ...
                                                <p className="font-anton text-xl tracking-wider text-[var(--theme-accent)] flex-shrink-0">
                                                    {fmt(
                                                        Number(item.price) *
                                                            item.quantity,
                                                    )}
                            ...
                            <div className="bg-white border-2 border-black shadow-[4px_4px_0_rgba(0,0,0,1)] p-8 mt-6">
                                ...
                                {order.status === "preparing" && (
                                    <p className="text-xs font-bold text-[#faad14] bg-[#faad14]/10 border-2 border-[#faad14] px-4 py-3 uppercase tracking-widest mb-6">
                                        ⚠️ Shop đang chuẩn bị hàng. Yêu cầu hủy
                                        sẽ được gửi cho shop để xem xét.
                                    </p>
                                )}
                                {!showCancelForm ? (
                                    <button
                                        onClick={() => setShowCancelForm(true)}
                                        className="px-6 py-3 border-2 border-black bg-white text-black text-xs font-bold uppercase tracking-widest hover:-translate-y-[2px] hover:shadow-[4px_4px_0_rgba(0,0,0,1)] transition-all"
                                    >
                                        {order.status === "preparing"
                                            ? "Gửi yêu cầu hủy"
                                            : "Hủy đơn hàng"}
                    ...
                    <div className="w-full lg:w-80 flex-shrink-0 space-y-6">
                        ...
                        <div className="bg-white border-2 border-black shadow-[4px_4px_0_rgba(0,0,0,1)] p-6">
                            <h3 className="font-anton text-xl uppercase tracking-widest text-black mb-6">
                                Thông tin đơn hàng
                            </h3>
                            <div className="space-y-4">
                                ...
                                <div className="flex items-center justify-between">
                                    ...
                                    <span
                                        className={`text-sm font-bold uppercase tracking-widest ${order.paymentStatus === "paid" ? "text-[#52c41a]" : "text-[#ff8c00]"}`}
                                    >
                        ...
                        <div className="bg-white border-2 border-black shadow-[4px_4px_0_rgba(0,0,0,1)] p-6">
                            ...
                            <div className="space-y-4">
                                ...
                                {Number(order.discount) > 0 && (
                                    <div className="flex items-center justify-between">
                                        <span className="text-xs font-bold uppercase tracking-widest text-[#52c41a]">
                                            Giảm giá
                                        </span>
                                ...
                                <div className="border-t-2 border-black/10 pt-4 mt-4 flex items-end justify-between">
                                    ...
                                    <span className="font-anton text-3xl tracking-widest text-black leading-none">
                                        {fmt(order.total)}
                                    </span>
                                </div>
                            </div>
                    ...
                    <div className="bg-white max-w-md w-full overflow-hidden border-2 border-black shadow-[8px_8px_0_rgba(0,0,0,1)]">
                        ...
                        <div className="bg-[var(--theme-accent)] px-6 py-4 text-black flex items-center justify-between border-b-2 border-black">
                            ...
                            <button
                                ...
                                className="text-black hover:scale-110 transition-transform text-xl font-bold cursor-pointer"
                            >
                                ✕
                            </button>
                        </div>
                        ...
                        <div className="p-6 space-y-6">
                            ...
                            <div className="flex items-center gap-4 bg-[#f8f8f8] p-4 border-2 border-black">
                                <div className="w-16 h-16 border-2 border-black bg-white overflow-hidden flex-shrink-0 flex items-center justify-center">
                                    {reviewProduct.productImage ? (
                                        <img
                                            src={
                                                reviewProduct.productImage.startsWith(
                                                    "http",
                                                )
                                                    ? reviewProduct.productImage
                                                    : `${API_URL}${reviewProduct.productImage}`
                                            }
                                ...
                                <div className="flex-1 min-w-0">
                                    <h4 className="font-bold text-black text-sm uppercase tracking-widest truncate">
                                        {reviewProduct.productTitle}
                                    </h4>
                                    {reviewProduct.color && (
                                        <p className="text-[10px] font-bold uppercase tracking-widest text-black/50 mt-1">
                                            MÀU: {reviewProduct.color}
                                        </p>
                                    )}
                                </div>
                            </div>
                            ...
                            {rewardInfo ? (
                                ...
                                <div className="text-center py-6 space-y-6">
                                    ...
                                    <div>
                                        <h4 className="text-2xl font-anton uppercase tracking-widest text-black">
                                            ĐÁNH GIÁ THÀNH CÔNG!
                                        </h4>
                                        <p className="text-xs font-bold uppercase tracking-widest text-black/60 mt-2">
                                            CẢM ƠN BẠN ĐÃ ĐÓNG GÓP ĐÁNH GIÁ SẢN
                                            PHẨM.
                                        </p>
                                    </div>
                                    <div className="bg-[#f8f8f8] border-2 border-black p-6 space-y-4 text-left">
                                        <div className="flex items-center justify-between">
                                            <span className="text-xs font-bold uppercase tracking-widest text-black/60">
                                                XU TÍCH LŨY NHẬN ĐƯỢC:
                                            </span>
                                            <span className="text-sm font-bold uppercase tracking-widest text-[#52c41a]">
                                                🪙 +{rewardInfo.points} XU
                                            </span>
                                        </div>
                                ...
                                <div className="space-y-6 text-left">
                                    <div>
                                        <label className="block text-xs font-bold text-black uppercase tracking-widest mb-3">
                                            CHỌN SỐ SAO
                                        </label>
