export const Button = ({ children, loading, variant = "primary", className = "", ...props }) => {
    const base = "w-full py-3.5 rounded-xl font-bold transition-all disabled:opacity-60 disabled:cursor-not-allowed tracking-wide";
    const variants = {
        primary: "bg-[var(--theme-accent)] text-black shadow-[0_18px_40px_rgba(255,122,26,0.28)] hover:brightness-110",
        secondary: "bg-white/5 text-[var(--theme-text)] border border-white/10 hover:bg-white/10",
...
export const InputField = ({ label, error, ...props }) => (
    <div className="mb-4">
        {label && <label className="block text-sm font-medium text-[var(--theme-text)] mb-1.5">{label}</label>}
        <input
...
export const Message = ({ text, type = "info" }) => {
    ...
    return (
        <div className={`mt-4 px-4 py-3 rounded-xl border text-sm text-center ${styles[type]}`}>
            {text}
        </div>
    );
};
...
export const Card = ({ children, className = "" }) => (
    <div className={`max-w-md w-full bg-[rgba(21,21,26,0.92)] p-8 rounded-[28px] shadow-[0_24px_80px_rgba(0,0,0,0.55)] border border-white/10 backdrop-blur-sm ${className}`}>
        {children}
    </div>
);

export const PageWrapper = ({ children, center = true }) => (
    <div className={`min-h-screen flex ${center ? "items-center justify-center" : "items-start justify-start"} px-4 py-12`}>
        {children}
    </div>
);
