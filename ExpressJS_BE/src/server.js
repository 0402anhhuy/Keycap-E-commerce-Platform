    ...
    try {
        ...
        try {
            await sequelize.query(
                "ALTER TABLE products MODIFY COLUMN status ENUM('active', 'draft', 'hidden', 'deleted', 'pending', 'rejected') NOT NULL DEFAULT 'active';",
            );
        } catch (e) {
            console.error(
                ">>> Error altering products status ENUM:",
                e.message,
            );
        }
        ...
        server.on("error", (err) => {
            if (err.code === "EADDRINUSE") {
                ...
                console.error(
                    `   Hoặc: netstat -ano | findstr :${PORT}  →  taskkill /PID <PID> /F\n`,
                );
                process.exit(1);
            } else {
